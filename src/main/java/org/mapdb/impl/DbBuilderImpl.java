    ...
    implements DBBuilder
{
    ...
    @Override
    public void assertNotInMemoryVolume() {
    ...
    /**
     * DB Get methods such as {@link org.mapdb.DB#getTreeMap(String)} or {@link org.mapdb.DB#getAtomicLong(String)} auto create
    ...
    public TxMaker makeTxMaker(){
        ...
        DB db = new DbImpl(e);
        db.commit();
        return new TxMakerImpl(e, propsGetBool(Keys.strictDBGet), propsGetBool(Keys.snapshots));
    }
    ...
    public Engine makeEngine(){
        ...
        try{
            check = (Fun.Tuple2<Integer, byte[]>) engine.get(Engine.CHECK_RECORD, SerializerBase.BASIC);
            if(check!=null){
        ...
        if(check == null && !engine.isReadOnly()){
            ...
            engine.update(Engine.CHECK_RECORD, check, SerializerBase.BASIC);
            engine.commit();
        }
