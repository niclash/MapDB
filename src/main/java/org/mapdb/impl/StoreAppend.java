...
public class StoreAppend extends Store
{
    ...
    public StoreAppend( final File file,
                        final boolean useRandomAccessFile,
                        final boolean readOnly,
                        final boolean transactionDisabled,
                        final boolean deleteFilesAfterClose,
                        final boolean syncOnCommitDisabled,
                        boolean checksum,
                        boolean compress,
                        byte[] password,
...
//                    System.out.println("replay "+recid+ " - "+pos);
    ...
    public <A> A get( long recid, ValueSerializer<A> serializer )
    {
        ...
        {
            lock = locks[ lockPos( recid ) ].readLock();
            lock.lock();
        }
        try
        {
    ...
    protected <A> A getNoLock( long recid, ValueSerializer<A> serializer )
        throws IOException
    {
        ...
        if( size == 0 )
        {
            return serializer.deserialize( new DataIO.DataInputByteArray( new byte[ 0 ] ), 0 );
        }
    ...
    public <A> void update( long recid, A value, ValueSerializer<A> serializer )
    {
        ...
        {
            lock = locks[ lockPos( recid ) ].writeLock();
            lock.lock();
        }
        try
        {
    ...
    public void close()
    {
        ...
        { //TODO and modified since last open
            rollover();
