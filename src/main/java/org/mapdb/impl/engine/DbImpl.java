...
import org.mapdb.impl.btree.BTreeKeySet;
import org.mapdb.impl.btree.BTreeMapImpl;
import org.mapdb.impl.btree.BTreeMapMakerImpl;
import org.mapdb.impl.btree.BTreeSetMakerImpl;
import org.mapdb.impl.htree.HTreeKeySet;
import org.mapdb.impl.htree.HTreeMapImpl;
import org.mapdb.impl.htree.HTreeMapMakerImpl;
import org.mapdb.impl.htree.HTreeSetMakerImpl;
import org.mapdb.impl.queue.CircularQueue;
import org.mapdb.impl.queue.FifoQueue;
import org.mapdb.impl.queue.SimpleQueue;
import org.mapdb.impl.queue.Stack;
...
public class DbImpl implements Closeable, DB
{
     ...
     * @throws IllegalArgumentException if name is already used
     */
    synchronized public <K, V> HTreeMap<K, V> createHashMap( HTreeMapMakerImpl m )
    {
        String name = m.name;
        ...
        if( m.expire != 0 || m.expireAccess != 0 || m.expireMaxSize != 0 || m.expireStoreSize != 0 )
        {
            ...
            for( int i = 0; i < 16; i++ )
            {
                expireHeads[ i ] = engine.put( 0L, InternalSerializers.LONG );
                expireTails[ i ] = engine.put( 0L, InternalSerializers.LONG );
            }
        ...
        HTreeMap<K, V> ret = new HTreeMapImpl<K, V>( engine,
                                                 catPut( name + ".counterRecid", !m.counter ? 0L : engine.put( 0L, InternalSerializers.LONG ) ),
    ...
    synchronized public <K> Set<K> createHashSet( HTreeSetMakerImpl m )
    {
        String name = m.name;
        checkNameNotExists( name );
    ...
    synchronized public <K, V> BTreeMap<K, V> createTreeMap( final BTreeMapMakerImpl m )
    {
        String name = m.name;
        checkNameNotExists( name );
        ...
        long counterRecid = !m.counter ? 0L : engine.put( 0L, InternalSerializers.LONG );

        long rootRecidRef;
        if( m.pumpSource == null )
        {
            rootRecidRef = BTreeMapImpl.createRootRef( engine, m.keySerializer, m.valueSerializer, 0 );
        }
        else
        {
    ...
    synchronized public <K> NavigableSet<K> createTreeSet( BTreeSetMakerImpl m )
    {
        ...
        if( m.pumpSource == null )
        {
            rootRecidRef = BTreeMapImpl.createRootRef( engine, m.serializer, null, 0 );
        }
        else
        {
            rootRecidRef = Pump.buildTreeMap(
    ...
    {
        checkNotClosed();
        FifoQueue<E> ret = (FifoQueue<E>) getFromWeakCollection( name );
        if( ret != null )
        {
            return ret;
        }
    ...
    @Override synchronized public <E> BlockingQueue<E> createQueue( String name, ValueSerializer<E> serializer, boolean useLocks )
    {
        ...
        long node = engine.put( SimpleQueue.Node.EMPTY, new SimpleQueue.NodeSerializer( serializer ) );
        long headRecid = engine.put( node, InternalSerializers.LONG );
        long tailRecid = engine.put( node, InternalSerializers.LONG );

        FifoQueue<E> ret = new FifoQueue<E>( engine,
    ...
    {
        checkNotClosed();
        Stack<E> ret = (Stack<E>) getFromWeakCollection( name );
        if( ret != null )
        {
            return ret;
        }
    ...
    @Override synchronized public <E> BlockingQueue<E> createStack( String name, ValueSerializer<E> serializer, boolean useLocks )
    {
        ...
        Stack<E> ret = new Stack<E>( engine,
                                                   catPut( name + ".serializer", serializer, getDefaultSerializer() ),
                                                   catPut( name + ".headRecid", headRecid ),
                                                   catPut( name + ".useLocks", useLocks )
        );
    ...
    @Override synchronized public <E> BlockingQueue<E> getCircularQueue( String name )
    {
        ...
        ret = new CircularQueue<E>( engine,
                                           (ValueSerializer<E>) catGet( name + ".serializer", getDefaultSerializer() ),
                                           (Long) catGet( name + ".headRecid" ),
...
//        long headerRecid = engine.put(0L, Serializer.LONG);
        ...
        long prevRecid = 0;
        long firstRecid = 0;
        ValueSerializer<SimpleQueue.Node<E>> nodeSer = new SimpleQueue.NodeSerializer<E>( serializer );
        for( long i = 0; i < size; i++ )
        {
            SimpleQueue.Node<E> n = new SimpleQueue.Node<E>( prevRecid, null );
        ...
        engine.update( firstRecid, new SimpleQueue.Node<E>( prevRecid, null ), nodeSer );

        long headRecid = engine.put( prevRecid, InternalSerializers.LONG );
        long headInsertRecid = engine.put( prevRecid, InternalSerializers.LONG );

        CircularQueue<E> ret = new CircularQueue<E>( engine,
    ...
    {
        checkNameNotExists( name );
        long recid = engine.put( initValue, InternalSerializers.LONG );
    ...
    {
        checkNameNotExists( name );
        long recid = engine.put( initValue, InternalSerializers.INTEGER );
    ...
    {
        checkNameNotExists( name );
        long recid = engine.put( initValue, InternalSerializers.BOOLEAN );
    ...
    @Override synchronized public Atomic.String createAtomicString( String name, String initValue )
    {
        ...
        long recid = engine.put( initValue, InternalSerializers.STRING_NOSIZE );
    ...
    @Override synchronized public void delete( String name )
    {
        ...
        {
            engine.delete( ( (Atomic.Boolean) r ).getRecid(), InternalSerializers.BOOLEAN );
        }
        else if( r instanceof Atomic.Integer )
        {
            engine.delete( ( (Atomic.Integer) r ).getRecid(), InternalSerializers.INTEGER );
        }
        else if( r instanceof Atomic.Long )
        {
            engine.delete( ( (Atomic.Long) r ).getRecid(), InternalSerializers.LONG );
        }
        else if( r instanceof Atomic.String )
        {
            engine.delete( ( (Atomic.String) r ).getRecid(), InternalSerializers.STRING_NOSIZE );
        }
        else if( r instanceof Atomic.Var )
        {
            engine.delete( ( (Atomic.Var) r ).getRecid(), ( (Atomic.Var) r ).serializer );
        }
        ...
        else if( r instanceof HTreeMap || r instanceof HTreeKeySet )
        {
            HTreeMapImpl m = ( r instanceof HTreeMapImpl ) ? (HTreeMapImpl) r : (HTreeMapImpl) ((HTreeKeySet) r ).parent();
            m.clear();
            //delete segments
        ...
        else if( r instanceof BTreeMap || r instanceof BTreeKeySet )
        {
            BTreeMapImpl m = ( r instanceof BTreeMapImpl ) ? (BTreeMapImpl) r : (BTreeMapImpl) ( (BTreeKeySet) r ).m;
            ...
            if( m.counter != null )
            {
                engine.delete( m.counter.getRecid(), InternalSerializers.LONG );
            }
        }
