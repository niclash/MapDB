...
public class TxEngine extends EngineWrapper
{
    ...
    protected <A> void superUpdate(long recid, A value, ValueSerializer<A> serializer) {
        assert(commitLock.isWriteLockedByCurrentThread());
        super.update(recid,value,serializer);
    }

    protected <A> void superDelete(long recid, ValueSerializer<A> serializer) {
        assert(commitLock.isWriteLockedByCurrentThread());
        super.delete(recid,serializer);
    }

    protected <A> A superGet(long recid, ValueSerializer<A> serializer) {
        assert(commitLock.isWriteLockedByCurrentThread());
        return super.get(recid,serializer);
    }
    ...
    private <A> A getNoLock(long recid, ValueSerializer<A> serializer) {
        if(fullTx){
    ...
    public void commit() {
        ...
        try{
            ...
            while(iter.moveToNext()){
                ...
                ValueSerializer ser = (ValueSerializer) val.b;
