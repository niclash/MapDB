...
public final class Pump {
    ...
    static void copy(DB db1, DbImpl db2){
        copy( Store.forDB( db1 ), Store.forDB(db2));
        db2.getEngine().clearCache();
        db2.reinit();
    }
    ...
    public static <E> Iterator<E> sort(Iterator<E> source, boolean mergeDuplicates, final int batchSize,
            Comparator comparator, final ValueSerializer serializer){
        if(batchSize<=0) throw new IllegalArgumentException();
        if(comparator==null)
            comparator= BTreeMapImpl.COMPARABLE_COMPARATOR;
        if(source==null)
    ...
    public static <E> Iterator<E> sort(Comparator comparator, final boolean mergeDuplicates, final Iterator... iterators) {
        final Comparator comparator2 = comparator==null?BTreeMapImpl.COMPARABLE_COMPARATOR:comparator;
    ...
    public static  <E,K,V> long buildTreeMap(Iterator<E> source,
        ...
        while(source.hasNext()){
            ...
            nodeLoop:for(int i=0;i<nload && source.hasNext();i++){
                ...
                Object val = valueExtractor!=null?valueExtractor.run(next):BTreeMapImpl.EMPTY;
