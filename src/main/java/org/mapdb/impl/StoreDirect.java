 ...
 * 15          | {@link StoreDirect#IO_FREE_RECID} |Long Stack of deleted recids, those will be reused and returned by {@link org.mapdb.Engine#put(Object, org.mapdb.ValueSerializer)}
...
public class StoreDirect extends Store
{
    ...
    public long preallocate()
    {
        ...
        try
        {
            ...
            {
                lock = locks[ lockPos( ioRecid ) ].writeLock();
                lock.lock();
            }
            try
            {
    ...
    {
        assert ( disableLocks || locks[ lockPos( ioRecid ) ].writeLock().isHeldByCurrentThread() );
        index.putLong( ioRecid, indexVals[ 0 ] | MASK_ARCHIVE );
        //write stuff
        if( indexVals.length == 1 || indexVals[ 1 ] == 0 )
        { //is more then one? ie linked
            //write single
    ...
    public <A> A get( long recid, ValueSerializer<A> serializer )
    {
        ...
        {
            lock = locks[ lockPos( ioRecid ) ].readLock();
            lock.lock();
        }
        try
        {
    ...
    protected <A> A get2( long ioRecid, ValueSerializer<A> serializer )
        throws IOException
    {
        assert ( disableLocks || locks[ lockPos( ioRecid ) ].getWriteHoldCount() == 0 ||
    ...
    public void compact()
    {
        ...
        try
        {
            ...
            {
                byte[] bb = get2( ioRecid, SerializerBase.BYTE_ARRAY_NOSIZE );
                ...
                {
                    DataOutput2 out = serialize( bb, SerializerBase.BYTE_ARRAY_NOSIZE );
    ...
    {
        //TODO use direct BB
        byte[] bb = get( recid, SerializerBase.BYTE_ARRAY_NOSIZE );
        if( bb == null )
        {
            return null;
        }
    ...
    public void updateRaw( long recid, ByteBuffer data )
    {
        ...
        update( recid, b, SerializerBase.BYTE_ARRAY_NOSIZE );
    }
